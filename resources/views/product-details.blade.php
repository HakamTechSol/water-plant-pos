<?php $page="Product Details - Pure Water";?>
@extends('layout.mainlayout')
@section('content')
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
<script>
        window.jsPDF = window.jspdf.jsPDF;
        // Convert HTML content to PDF
        function Convert_HTML_To_PDF() {
            var doc = new jsPDF();
            
            // Source HTMLElement or a string containing HTML.
            var elementHTML = document.querySelector("#barcode");
            // console.log(elementHTML);

            doc.setProperties({
                title: "Bar Code"
            });
            doc.html(elementHTML, {
                callback: function(doc) {
                    // Save the PDF
                    doc.output('dataurlnewwindow');
                },
                margin: [10, 10, 10, 10],
                autoPaging: 'text',
                x: 0,
                y: 0,
                width: 2000, //target width in the PDF document
                windowWidth: 2000 //window width in CSS pixels
            });
        }
</script>
<div class="page-wrapper">
    <div class="content">
        @component('components.pageheader')                
			@slot('title') Product Details @endslot
			@slot('title_1') Full details of a product @endslot
		@endcomponent
        <!-- /add -->
       
        <div class="row">
            <div class="col-lg-8 col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <div class="bar-code-view">
                            <?php 
                            // $productbarcode = $product->product_name.(string)$product->product_id ;
                            $productbarcode =$product->product_id ;
                            ?>
                            <div class="barcode" id="barcode">
                            {!! DNS1D::getBarcodeHTML($productbarcode, "C128",1.4,22) !!}
                            {{--<p class="pid">{{$product->product_id}}</p>--}}
                            </div>
                            <!-- <img src="{{ URL::asset('/assets/img/barcode1.png')}}" alt="barcode"> -->
                            <a class="printimg" onclick="Convert_HTML_To_PDF()">
                                <img src="{{ URL::asset('/assets/img/icons/printer.svg')}}" alt="print">
                            </a>
                        </div>
                        <div class="productdetails">
                            <ul class="product-bar">
                                <li>
                                    <h4>Product</h4>
                                    <h6>{{$product->product_name}}	</h6>
                                </li>
                                <li>
                                    <h4>Category</h4>
                                    <h6>{{$categorys->category_name}}</h6>
                                </li>
                                <li>
                                    <h4>Size</h4>
                                    <h6>{{ $size->size_name }}</h6>
                                </li>
                                <li>
                                    <h4>Brand</h4>
                                    <h6>{{ $brands->brand_name }}</h6>
                                </li>
                                <li>
                                    <h4>Unit</h4>
                                    <h6>{{$product->product_unit}}</h6>
                                </li>
                                <li>
                                    <h4>SKU</h4>
                                    <h6>{{$product->product_SKU}}</h6>
                                </li>
                                {{-- <li>
                                    <h4>Minimum Qty</h4>
                                    <h6>{{$product->product_min_qty}}</h6>
                                </li> --}}
                                <li>
                                    <h4>Quantity</h4>
                                    <h6>{{$product->product_qty}}</h6>
                                </li>
                               
                                <li>
                                    <h4>Price</h4>
                                    <h6>PKR {{$product->product_price}}</h6>
                                </li>
                                
                                <li>
                                    <h4>Description</h4>
                                    <h6>{{ $product->product_desc}}</h6>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <div class="slider-product-details">
                            <div class="owl-carousel owl-theme product-slide">
                                <div class="slider-product">
                                <img src="{{ URL::asset('storage/product_img/'.$product->product_img)}}" alt="img">

                                    <h4>{{$product->product_img}}</h4>
                                    <!-- <h6>581kb</h6> -->
                                </div>
                                <!-- <div class="slider-product">
                                    <img src="{{ URL::asset('/assets/img/product/product69.jpg')}}" alt="img">
                                    <h4>macbookpro.jpg</h4>
                                     <h6>581kb</h6> 
                                </div> -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
            
        <!-- /add -->
    </div>
</div>
@endsection